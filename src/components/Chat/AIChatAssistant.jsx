...
const AIChatAssistant = () => {
  ...
  const [conversationType, setConversationType] = useState("ai"); // 'ai', 'advisor', 'announcement'
...
const fetchConversations = async () => {
  ...
  const loadConversation = async (conversation) => {
    ...
    if (conversation.title === 'محادثة مع المشرف الأكاديمي' ||
      ...
      setConversationType('advisor');
             ...
             conversation.title?.toLowerCase().includes('اعلان')) {
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden">
      ...
      {/* Main Chat Area - تصميم مختلف حسب نوع المحادثة */}
      <div className="flex-1 flex flex-col min-w-0">
        
        {/* ========== تصميم محادثة المشرف الأكاديمي (Advisor Chat) - قراءة فقط ========== */}
        {conversationType === 'advisor' && (
          <div className="flex flex-col h-full bg-gray-100">
            ...
            {/* Messages Area - واتساب style */}
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#efeae2]">
              {messages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center text-gray-500">
                  <FaUserTie className="text-5xl mb-3 opacity-40" />
                  <p className="text-sm">No messages yet</p>
                  <p className="text-xs">Start the conversation with your advisor</p>
                </div>
              ) : (
            ...
            {/* View-only message (بدون Text Input) */}
            <div className="p-4 bg-gray-50 border-t text-center">
              <p className="text-xs text-gray-400">
                📖 This conversation is for viewing only. You cannot reply here.
              </p>
            </div>
          </div>
        )}

        {/* ========== تصميم الإعلانات (Announcement) - قراءة فقط ========== */}
        {conversationType === 'announcement' && (
          <div className="flex flex-col h-full bg-gradient-to-br from-amber-50 to-orange-50">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-amber-50/50">
              {messages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center text-amber-600">
                  <FaRobot className="text-5xl mb-3 opacity-40" />
                  <p className="text-sm">No announcements yet</p>
                  <p className="text-xs">Check back later for important updates</p>
                </div>
              ) : (
                messages.map((message) => (
                  <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
            ...
            <div className="p-4 bg-amber-50 border-t text-center">
              <p className="text-xs text-amber-600">
                📢 This is an announcement. No replies needed.
              </p>
            </div>
          </div>
        )}

        {/* ========== تصميم AI Chat (مع الـ Input) ========== */}
        {conversationType === 'ai' && (
          <div className="flex flex-col h-full bg-white">
            ...
            {/* Input Area موجود فقط في AI Chat */}
